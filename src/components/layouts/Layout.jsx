...
export default function Layout({ children }) {
  ...
  const label = children.type.name.toLowerCase() || ""
  console.log(label)
  ...
  return (
    ...
    >
      <header className="w-full h-auto flex flex-col gap-3 justify-start items-center bg-white relative z-40">
        ...
        <div className="relative z-40 w-full h-1 bg-danger"></div>
        <p className="relative z-40 w-full h-2/3 text-md shadow-xl p-3 bg-danger text-off-white rounded-lg shadow-md">{headerStyles[label]["description"]}</p>
      </header>
      {children}
    </section>
  );
